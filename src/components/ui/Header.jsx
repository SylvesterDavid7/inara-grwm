  ...
  const navigationItems = [
    {
      label: 'Analyze',
      items: [
        { label: 'Routine Input', path: '/skincare-routine-input', icon: 'Plus', requiresAuth: true },
        { label: 'Skin Assessment', path: '/skin-assessment-questionnaire', icon: 'ClipboardList', requiresAuth: true }
      ]
    },
    {
      label: 'Results',
      items: [
        { label: 'Scorecard', path: '/skincare-scorecard-results', icon: 'LayoutDashboard', requiresAuth: true },
        { label: 'Recommendations', path: '/product-recommendations', icon: 'Star', requiresAuth: true }
      ]
    },
    {
      label: 'Track',
      items: [
        { label: 'Dashboard', path: '/progress-tracking-dashboard', icon: 'TrendingUp', requiresAuth: true }
      ]
    },
    {
      label: 'Learn',
      items: [
        { label: 'Ingredients', path: '/ingredient-education-hub', icon: 'BookOpen', requiresAuth: false },
        { label: 'Skincare 101', path: '/skincare-101', icon: 'Grid3X3', requiresAuth: false }
      ]
    }
  ];
                      ...
                      <div className="py-2">
                        {section?.items?.map((item) => (
                          <button
                            ...
                            className={`flex items-center space-x-3 px-4 py-2 text-sm transition-clinical w-full text-left ${
                  ...
                  <div className="space-y-1">
                    {section.items.map((item) => (
                      <button
                        key={item.path}
                        onClick={() => handleLinkClick(item)}
                        className={`group flex items-center space-x-3 p-3 rounded-clinical-md text-base transition-colors w-full text-left ${
                          isActivePath(item.path)
                            ? 'bg-secondary'
